...
def convert_webui_to_api(workflow):
    ...
    Handles: SetNode/GetNode resolution, subgraph flattening, widgets_values mapping.
    """
    ...
    # Resolve SetNode/GetNode pairs: GetNode references SetNode by name,
    # SetNode's input link is the actual source. Replace GetNode references
    # with the real source node.
    set_nodes = {}  # name -> source [node_id, slot]
    for node in workflow.get("nodes", []):
        if node.get("type") == "SetNode":
            name = (node.get("widgets_values") or [""])[0]
            for inp in node.get("inputs", []):
                if inp.get("link") is not None and inp["link"] in link_map:
                    set_nodes[name] = link_map[inp["link"]]
                    break

    get_node_resolve = {}  # get_node_id -> [real_src_id, real_src_slot]
    for node in workflow.get("nodes", []):
        if node.get("type") == "GetNode":
            name = (node.get("widgets_values") or [""])[0]
            if name in set_nodes:
                get_node_resolve[str(node["id"])] = set_nodes[name]

    # Rewrite link_map: any link whose source is a GetNode should point
    # to the real source instead
    for lid, (src_id, src_slot) in list(link_map.items()):
        if src_id in get_node_resolve:
            link_map[lid] = list(get_node_resolve[src_id])
    ...
    for node in workflow.get("nodes", []):
        ...
        if ntype in subgraphs_def:
            ...
            # Build map of external input connections for this subgraph instance
            ext_input_links = {}
            for inp in node.get("inputs", []):
                if inp.get("link") is not None:
                    ext_input_links[inp["name"]] = inp["link"]

            # Map subgraph input names to their external sources
            sg_input_sources = {}  # sg_input_slot -> [real_src_id, real_src_slot]
            for idx, sg_inp in enumerate(sg.get("inputs", [])):
                name = sg_inp.get("name")
                if name in ext_input_links:
                    ext_lid = ext_input_links[name]
                    if ext_lid in link_map:
                        sg_input_sources[idx] = link_map[ext_lid]

            # Map inner links, resolving -10 (input placeholder) references
            for link in sg.get("links", []):
                ...
                if src_id == -10:
                    # -10 means "from subgraph input slot src_slot"
                    if src_slot in sg_input_sources:
                        link_map[link_id] = list(sg_input_sources[src_slot])
                    # else: unconnected input, leave unmapped
                else:
                ...
                if inner_links and inner_links[0] in link_map:
                    ...
                    if name in ext_output_links:
                        for ext_lid in ext_output_links[name]:
                            link_map[ext_lid] = list(inner_src)
    ...
    # Collect "Anything Everywhere" broadcasts: type -> [src_node_id, src_slot]
    ae_broadcasts = {}
    for node in workflow.get("nodes", []):
        if node.get("type") == "Anything Everywhere":
            for inp in node.get("inputs", []):
                itype = inp.get("type", "*")
                lid = inp.get("link")
                if lid is not None and lid in link_map and itype != "*":
                    ae_broadcasts[itype] = link_map[lid]

    SKIP_TYPES = {"Reroute", "Note", "PrimitiveNode", "SetNode", "GetNode",
                  "Anything Everywhere", "MarkdownNote"}
    ...
    def has_extra_widget(spec):
        if len(spec) < 2 or not isinstance(spec[1], dict):
            return False
        cfg = spec[1]
        return bool(cfg.get("control_after_generate") or cfg.get("image_upload"))

    def validate_value(val, spec):
        if isinstance(spec[0], list) and val not in spec[0]:
            return spec[0][0] if spec[0] else val
        if len(spec) < 2 or not isinstance(spec[1], dict):
            return val
        cfg = spec[1]
        try:
            if "min" in cfg and "max" in cfg:
                num = float(val)
                if num < cfg["min"] or num > cfg["max"]:
                    return cfg.get("default", val)
    ...
    for node in flat_nodes:
        ...
        for name, spec in all_inputs:
            if name in input_links:
                ...
                elif is_widget_type(spec) and wi < len(wv):
                    val = wv[wi]
                    if val is not None:
                        val = validate_value(val, spec)
                        api_inputs[name] = val
                ...
                # Unconnected non-widget input — try Anything Everywhere broadcast
                itype = spec[0] if isinstance(spec[0], str) else None
                if itype and itype in ae_broadcasts and name not in api_inputs:
                    api_inputs[name] = list(ae_broadcasts[itype])
    ...
    if wf and Path(wf).exists():
        try:
            with open(wf, encoding="utf-8") as f:
                mf = json.load(f)
            if mf.get("is_master"):
                data["chunk_files"] = mf.get("chunks", [])
        except Exception:
            pass
    return jsonify(data)
...
def generate():
    ...
    master_data = {
        "is_master": True,
        "chunks": chunk_files,
        "chunk_status": {cf: "pending" for cf in chunk_files},
        "config": config,
        "created": ts,
    }
    ...
    if channel_id:
        ...
        video_id = None
    return jsonify({"output_file": str(out_file), "chunk_files": chunk_files,
                    "video_id": video_id, **info})
...
def _get_chunk_output_prefix(chunk_path):
    """Extract the SaveVideo filename_prefix from a chunk workflow."""
    try:
...
def _check_chunk_completed(chunk_path):
    """Check if a chunk's output video exists in ComfyUI output directory."""
    prefix = _get_chunk_output_prefix(chunk_path)
    if not prefix:
        return False
    comfy_output = COMFYUI_DIR / "ComfyUI" / "output"
    # prefix like "%date:yyyy-MM-dd%/LTX-2_3_chunk0" — the date part is a ComfyUI
    # template, so we search for files matching the static part after the last /
    static_part = prefix.split("/")[-1] if "/" in prefix else prefix
    for f in comfy_output.rglob(f"{static_part}_*.*"):
        if f.suffix in (".mp4", ".webm", ".avi"):
            return True
    return False

def _update_master_status(master_path, chunk_path, status):
    """Update a chunk's status in the master file."""
    with open(master_path, encoding="utf-8") as f:
        master = json.load(f)
    if "chunk_status" not in master:
        master["chunk_status"] = {}
    master["chunk_status"][chunk_path] = status
    with open(master_path, "w", encoding="utf-8") as f:
        json.dump(master, f, indent=2)

@app.route("/api/comfyui/check-chunks", methods=["POST"])
def check_chunks():
    """Check which chunks have completed output files."""
    d = request.get_json()
    ...
    if not master.get("is_master"):
        return jsonify({"error": "Not a master workflow file"}), 400
    chunks = master.get("chunks", [])
    chunk_status = master.get("chunk_status", {})
    results = []
    for cp in chunks:
        completed = _check_chunk_completed(cp)
        status = "completed" if completed else chunk_status.get(cp, "pending")
        if completed and chunk_status.get(cp) != "completed":
            _update_master_status(wf_path, cp, "completed")
        results.append({
            "path": cp,
            "status": status,
            "exists": Path(cp).exists(),
            "prefix": _get_chunk_output_prefix(cp),
        })
    total = len(results)
    done = sum(1 for r in results if r["status"] == "completed")
    return jsonify({
        "total": total, "completed": done, "remaining": total - done,
        "chunks": results,
    })
...
def comfyui_run():
    ...
    d = request.get_json()
    wf_path = d.get("workflow_path", "")
    resume_only = d.get("resume", False)
    if not wf_path or not Path(wf_path).exists():
        return jsonify({"error": "Workflow file not found"}), 400
    ...
    try:
        ...
        if wf_data.get("is_master"):
            chunks = wf_data["chunks"]
            for chunk_path in chunks:
                if resume_only and _check_chunk_completed(chunk_path):
                    _update_master_status(wf_path, chunk_path, "completed")
                    skipped += 1
                    continue
                ...
                _update_master_status(wf_path, chunk_path, "queued")
            queued = len(prompt_ids)
            total = len(chunks)
            if resume_only and skipped > 0:
                msg = f"{queued} chunks queued, {skipped} already completed (skipped). Total: {total}"
            else:
                msg = f"{queued} chunks queued in ComfyUI!"
        ...
        return jsonify({"ok": True, "queued": True, "prompt_id": prompt_id,
                        "queued_count": len(prompt_ids), "skipped_count": skipped,
                        "message": msg})
...
async function generateWorkflow(andRun=false) {
  ...
  try {
    ...
    const chunks = r.chunk_files || [];
    const loadPath = chunks.length === 1 ? chunks[0] : r.output_file;
    let chunkInfo = chunks.length > 1 ? `<br><small style="color:var(--tx2)">${chunks.length} chunk files (auto-queued sequentially)</small>` : '';
    el.innerHTML = `<span style="color:var(--ok)">Workflow saved:</span> ${esc(loadPath)}<br>
      ${r.clip_count} clips, ${r.total_seconds.toFixed(0)}s, ${r.resolution}${chunkInfo}
      <br><button class="btn btn-s btn-sm" style="margin-top:8px" onclick="copyPath('${loadPath.replace(/\\/g,'\\\\')}')">Copy Path</button>`;
...
async function openVideoDetail(id) {
  ...
  document.getElementById('vmContent').innerHTML = `
    ...
    ${v.workflow_path?`<div class="fg"><strong>Workflow:</strong> <code style="font-size:12px;color:var(--tx2)">${esc((v.chunk_files&&v.chunk_files.length===1)?v.chunk_files[0]:v.workflow_path)}</code> <button class="btn btn-s btn-sm" onclick="copyPath('${((v.chunk_files&&v.chunk_files.length===1)?v.chunk_files[0]:v.workflow_path).replace(/\\/g,'\\\\')}')">Copy</button></div>`:''}
...
# NOTE: duplicate HTML_TEMPLATE that was here has been removed
